...
export default class Repository {
  ...
  static getETag(modelName) {
    if (modelName in repositoryEtags) return repositoryEtags[modelName];
    else null;
  }
  newETag() {
  ...
  add(object) {
    delete object.Id;
    if (this.model.securedId) object = { Id: "", ...object };
    else object = { Id: 0, ...object };
  ...
  update(id, object, handleAssets = true) {
    ...
    if (!this.model.securedId) id = parseInt(id);
    ...
    if (this.model.state.isValid) {
      ...
      if (index > -1) {
        ...
        this.model.addError(
          `The ressource [${objectToModify.Id}] does not exist.`
        );
  ...
  getAll(params = null, dontBind = false) {
    ...
    if (objectsList)
      for (let data of objectsList) {
        if (dontBind) bindedDatas.push(data);
        else bindedDatas.push(this.model.bindExtraData(data));
      }
    ...
    if (collectionFilter.valid()) return collectionFilter.get();
    else {
  ...
  get(id, dontBind = false) {
    ...
    for (let object of this.objects()) {
      if (object.Id === id) {
        if (dontBind) return object;
        else return this.model.bindExtraData(object);
      }
    }
    return null;
  }
